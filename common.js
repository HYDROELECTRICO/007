...
function getProductPrice(product, variantIndex) {
  const variant = product.variants && product.variants[variantIndex || 0];
  return Number((variant && variant.price != null ? variant.price : product.price) || 0);
}
function productPriceHTML(product, variantIndex) {
  var price = getProductPrice(product, variantIndex || 0);
  if (!price) return '<div class="prod-enquiry">Enquire for availability</div>';
  var variant = product.variants && product.variants[variantIndex || 0];
  var was = Number(((variant && variant.original_price) || product.original_price) || 0);
  return '<div class="prod-price">' + formatINR(price) + (was > price ? ' <span class="was">' + formatINR(was) + '</span>' : '') + '</div>';
}
function priceLine(product, variantIndex) {
  var price = getProductPrice(product, variantIndex || 0);
...
function headerHTML() {
  ...
  '<div class="header-main">' +
    ...
    '<div class="search-bar" role="search">' +
      '<input type="text" id="siteSearch" aria-label="Search products" role="combobox" aria-autocomplete="list" aria-expanded="false" placeholder="Search for wires, fans, switches, appliances..." data-i18n-ph="searchPh"/>' +
  ...
  '<nav class="main-nav">' +
    '<div class="nav-inner">' +
      ...
      '<div class="nav-item"><a class="nav-link" href="compare.html">Compare</a></div>' +
      '<div class="nav-item"><a class="nav-link" href="dealer-enquiry.html">Dealer RFQ</a></div>' +
...
function footerHTML() {
  ...
  '<div class="footer-inner">' +
    ...
    '<div class="footer-col"><h4 data-i18n="ftQuick">Quick Links</h4><ul>' +
      ...
      '<li><a href="dealer-enquiry.html">Dealer / Bulk RFQ</a></li>' +
...
function cartTotal() {
  return getCart().reduce(function(s, i) { return s + (Number(i.price || 0) * i.qty); }, 0);
}
function renderCartDrawer() {
    ...
    wrap.innerHTML = cart.map(function(item, i) {
      return '<div class="cart-item">' +
        ...
        '<div class="cart-item-info">' +
          ...
          (item.price ? '<div class="cart-item-price">' + formatINR(item.price) + '</div>' : '') +
  ...
  if (totalRow) totalRow.style.display = total ? 'flex' : 'none';
...
function orderCartWA() {
  ...
  cart.forEach(function(item, i) {
    msg += '\n' + (i + 1) + ') ' + item.name +
      ...
      '\n   Qty: ' + item.qty + (item.price ? ' × ' + formatINR(item.price) + ' = ' + formatINR(item.price * item.qty) : '');
  });
  if (cartTotal()) msg += '\n\n' + (LANG === 'gu' ? 'કુલ: ' : 'Total: ') + formatINR(cartTotal());
...
function cartDrawerHTML() {
  ...
  '<aside class="cart-drawer" id="cartDrawer" aria-label="Shopping cart">' +
    ...
    '<div class="cart-foot" id="cartFoot" style="display:none;">' +
      '<div class="cart-total-row"><span data-i18n="cartTotal">Total:</span><span class="amount" id="cartTotalAmount">₹0</span></div>' +
      ...
      '<p class="cart-note">Your RFQ/enquiry is sent via WhatsApp. Prices are shown when available.</p>' +
    '</div>' +
  '</aside>' +
...
document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') { closeCart(); var m=document.getElementById('ekModal'); if(m) m.classList.remove('open'); }
});
...
function injectBusinessSchema() {
  ...
  s.textContent = JSON.stringify({
    ...
    'url': CONFIG.domain,
    'logo': CONFIG.domain + '/assets/icons/icon-512.png',
    'image': CONFIG.domain + '/assets/icons/icon-512.png',
    'sameAs': [],
...
function injectWebsiteSchema() {
  if (document.getElementById('websiteSchema')) return;
  var s = document.createElement('script');
  s.type = 'application/ld+json';
  s.id = 'websiteSchema';
  s.textContent = JSON.stringify({
    ...
    'potentialAction': {
      '@type': 'SearchAction',
      'target': CONFIG.domain + '/products.html?q={search_term_string}',
...
function injectBreadcrumbSchema() {
  if (location.pathname.split('/').pop() === 'product-detail.html') return;
  if (document.getElementById('pageBreadcrumbSchema') || document.getElementById('prodBreadcrumb')) return;
  var crumbs = [];
  var el = document.querySelector('.breadcrumb, #breadcrumb');
  if (el) {
    el.querySelectorAll('a, span').forEach(function(n) {
      var txt = (n.textContent || '').trim();
      if (!txt || txt === '›') return;
      crumbs.push({ text: txt, href: n.getAttribute('href') || window.location.href });
    });
  }
  if (!crumbs.length) crumbs = [{ text: 'Home', href: 'index.html' }, { text: document.title.replace(/\s*\|.*$/, ''), href: window.location.href }];
  var s = document.createElement('script');
  s.type = 'application/ld+json';
  s.id = 'pageBreadcrumbSchema';
  s.textContent = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    'itemListElement': crumbs.map(function(c, i) { return { '@type': 'ListItem', 'position': i + 1, 'name': c.text, 'item': c.href.indexOf('http') === 0 ? c.href : CONFIG.domain + '/' + c.href.replace(/^\//, '') }; })
  });
  document.head.appendChild(s);
}
...
/* ─────────── ENTERPRISE E-COMMERCE HELPERS ─────────── */
function ekLoadProducts(cb){
  if (window.EK_ALL_PRODUCTS && window.EK_ALL_PRODUCTS.length) { cb(window.EK_ALL_PRODUCTS); return; }
  fetch('products.json').then(function(r){return r.json();}).then(function(p){ p.forEach(function(x){ x._searchText=(x.name+' '+x.brand+' '+x.category+' '+(x.subcategory||'')+' '+(x.model_number||'')+' '+(x.product_code||'')+' '+(x.series||'')+' '+(x.product_type||'')+' '+((x.tags||[]).join(' '))+' '+((x.search_keywords||[]).join(' '))+' '+((x.seo_keywords||[]).join(' '))).toLowerCase(); }); window.EK_ALL_PRODUCTS=p; cb(p); }).catch(function(){ cb([]); });
}
function ekProductById(products,id){ id=parseInt(id,10); return products.find(function(p){return p.id===id;}); }
function ekIds(key){ try{return JSON.parse(localStorage.getItem(key))||[];}catch(e){return [];} }
function ekSaveIds(key,ids){ localStorage.setItem(key,JSON.stringify(Array.from(new Set(ids.map(Number))).filter(Boolean))); }
function toggleWishlist(id){ var ids=ekIds('ek_wishlist'); id=Number(id); ids=ids.includes(id)?ids.filter(function(x){return x!==id;}):ids.concat([id]); ekSaveIds('ek_wishlist',ids); showToast(ids.includes(id)?'Added to wishlist':'Removed from wishlist'); }
function shareWishlist(){ var ids=ekIds('ek_wishlist'); var url=location.origin+location.pathname+'?wishlist='+ids.join(','); navigator.clipboard&&navigator.clipboard.writeText(url); openWhatsApp('My ElectricalsKart wishlist:\n'+url); }
function loadWishlistFromUrl(){ var q=new URLSearchParams(location.search).get('wishlist'); if(q){ ekSaveIds('ek_wishlist',q.split(',').map(Number)); showToast('Wishlist loaded from shared link'); } }
function addCompare(id){ ekLoadProducts(function(products){ var p=ekProductById(products,id); if(!p)return; var ids=ekIds('ek_compare'); var selected=ids.map(function(x){return ekProductById(products,x);}).filter(Boolean); if(selected.length && selected[0].category!==p.category){ showToast('Compare only same category products'); return; } if(!ids.includes(Number(id))) ids.push(Number(id)); ekSaveIds('ek_compare',ids); showToast('Added to compare'); }); }
function goCompare(id){ if(id) addCompare(id); setTimeout(function(){location.href='compare.html';},120); }
function trackProductView(id){ var ids=ekIds('ek_recent_products').filter(function(x){return x!==Number(id);}); ids.unshift(Number(id)); ekSaveIds('ek_recent_products',ids.slice(0,24)); }
function renderProductStrip(containerId, ids, products){ var el=document.getElementById(containerId); if(!el)return; var items=(ids||[]).map(function(id){return ekProductById(products,id);}).filter(Boolean).slice(0,8); if(!items.length){ el.innerHTML='<p style="color:#64748b">No products yet.</p>'; return; } el.innerHTML='<div class="prod-grid">'+items.map(function(p){return '<div class="prod-card" onclick="location.href=\'product-detail.html?id='+p.id+'\'"><div class="prod-img"><img src="'+p.variants[0].image+'" onerror="this.parentElement.innerHTML=\'📦\'"/></div><div class="prod-body"><div class="prod-brand">'+p.brand+'</div><div class="prod-name">'+p.name+'</div>'+productPriceHTML(p,0)+'</div></div>';}).join('')+'</div>'; }
function compatibleIds(product, products, limit){ var base=new Set((product.tags||[]).map(function(t){return String(t).toLowerCase();})); return products.filter(function(p){return p.id!==product.id;}).map(function(p){var score=0; if(p.category===product.category)score+=4; if(p.brand===product.brand)score+=2; (p.tags||[]).forEach(function(t){ if(base.has(String(t).toLowerCase()))score++;}); if((product.frequently_bought_together||[]).includes(p.id))score+=10; if((product.related_accessories||[]).includes(p.id))score+=8; return {p:p,score:score};}).filter(function(x){return x.score>3;}).sort(function(a,b){return b.score-a.score;}).slice(0,limit||8).map(function(x){return x.p.id;}); }
function quickView(id){ ekLoadProducts(function(products){ var p=ekProductById(products,id); if(!p)return; var m=document.getElementById('ekModal')||ekModal(); var feats=(p.highlights||p.key_features||p.features||[]).slice(0,6).map(function(f){return '<li>'+f+'</li>';}).join(''); m.querySelector('.ek-modal-body').innerHTML='<div class="ek-qv"><div><img src="'+p.variants[0].image+'"/><div class="ek-chip-row">'+(p.tags||[]).slice(0,8).map(function(t){return '<span class="ek-chip">'+t+'</span>';}).join('')+'</div></div><div><h2>'+p.name+'</h2><p><b>'+p.brand+'</b> · '+p.category+'</p>'+productPriceHTML(p,0)+'<p>'+ (p.short_description||p.description||'') +'</p><ul>'+feats+'</ul><div class="ek-mini-actions"><button class="btn-dark" onclick="location.href=\'product-detail.html?id='+p.id+'\'">View Details</button><button class="btn-wa" onclick="openWhatsApp(\'Hello, I need a quote for: '+escAttr(p.name)+'\')">Ask Price</button><button class="ek-icon-btn" onclick="toggleWishlist('+p.id+')">♡ Wishlist</button><button class="ek-icon-btn" onclick="addCompare('+p.id+')">⇄ Compare</button></div></div></div>'; m.classList.add('open'); }); }
function ekModal(){ var m=document.createElement('div'); m.className='ek-modal'; m.id='ekModal'; m.innerHTML='<div class="ek-modal-card"><div class="ek-modal-head"><b>Quick View</b><button class="ek-close" onclick="document.getElementById(\'ekModal\').classList.remove(\'open\')">×</button></div><div class="ek-modal-body"></div></div>'; document.body.appendChild(m); m.addEventListener('click',function(e){if(e.target===m)m.classList.remove('open');}); return m; }
function requestQuote(name){ openWhatsApp('Hello, please share price/quote for:\n'+name+'\nQty: '); }
function requestCallback(productName){ var m=document.getElementById('ekModal')||ekModal(); m.querySelector('.ek-modal-body').innerHTML='<h2>Request Callback</h2><div class="ek-form-grid"><input id="cbName" placeholder="Your name"><input id="cbPhone" placeholder="Phone number"><input id="cbCompany" placeholder="Company / Dealer name (optional)"><input value="'+escAttr(productName||'General enquiry')+'" id="cbProduct"><textarea id="cbMsg" placeholder="Requirement"></textarea></div><button class="btn-dark" onclick="sendCallbackReq()">Send on WhatsApp</button>'; m.classList.add('open'); }
function sendCallbackReq(){ var msg='Callback request%0AName: '+encodeURIComponent(document.getElementById('cbName').value)+'%0APhone: '+encodeURIComponent(document.getElementById('cbPhone').value)+'%0AProduct: '+encodeURIComponent(document.getElementById('cbProduct').value)+'%0ACompany: '+encodeURIComponent(document.getElementById('cbCompany').value)+'%0ARequirement: '+encodeURIComponent(document.getElementById('cbMsg').value); window.open('https://wa.me/'+WA_PHONE+'?text='+msg,'_blank'); }
function shareTo(network,name){ var u=encodeURIComponent(location.href), t=encodeURIComponent(name||document.title); if(network==='whatsapp')window.open('https://wa.me/?text='+t+'%20'+u,'_blank'); else if(network==='facebook')window.open('https://www.facebook.com/sharer/sharer.php?u='+u,'_blank'); else if(network==='x')window.open('https://twitter.com/intent/tweet?text='+t+'&url='+u,'_blank'); else {navigator.clipboard&&navigator.clipboard.writeText(location.href); showToast('Link copied');} }
function downloadSpecPDF(product){ var w=window.open('','_blank'); if(!w)return; var rows=Object.entries(product.comparison_attributes||{}).map(function(r){return '<tr><th>'+r[0]+'</th><td>'+r[1]+'</td></tr>';}).join(''); w.document.write('<html><head><title>'+product.name+' Specs</title><style>body{font-family:Arial;padding:30px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px;text-align:left}h1{color:#0a2d6e}</style></head><body><h1>'+product.name+'</h1><p>'+product.short_description+'</p><table>'+rows+'</table><p>ElectricalsKart</p><script>window.print()<\/script></body></html>'); w.document.close(); }
function initSearchEnhancements(){ var input=document.getElementById('siteSearch'); if(!input)return; var box=document.createElement('div'); box.className='ek-suggestions'; box.setAttribute('role','listbox'); box.id='ekSearchSuggestions'; input.setAttribute('aria-controls','ekSearchSuggestions'); document.body.appendChild(box); function pos(){var r=input.getBoundingClientRect(); box.style.left=(r.left+scrollX)+'px'; box.style.top=(r.bottom+scrollY+6)+'px'; box.style.width=r.width+'px';} function showRecent(){ pos(); var recent=ekIds('ek_recent_searches').map(String); if(!recent.length){box.style.display='none'; input.setAttribute('aria-expanded','false'); return;} box.innerHTML='<div class="ek-suggest-item"><b>Recent searches</b></div>'+recent.map(function(q){return '<div class="ek-suggest-item" onclick="location.href=\'products.html?q='+encodeURIComponent(q)+'\'">🔎 '+q+'</div>';}).join(''); box.style.display='block'; input.setAttribute('aria-expanded','true'); }
  input.addEventListener('focus',function(){ if(!input.value.trim()) showRecent(); });
  input.addEventListener('input',function(){ var q=input.value.trim().toLowerCase(); pos(); if(!q){showRecent();return;} ekLoadProducts(function(products){ var res=products.filter(function(p){return (p._searchText || (p.name+' '+p.brand+' '+(p.search_keywords||[]).join(' ')+' '+(p.tags||[]).join(' ')).toLowerCase()).includes(q);}).slice(0,8); box.innerHTML=res.map(function(p){return '<div class="ek-suggest-item" onclick="location.href=\'product-detail.html?id='+p.id+'\'"><img loading="lazy" decoding="async" width="44" height="44" src="'+p.variants[0].image+'" alt=""><div><div class="ek-suggest-name">'+p.name+'</div><div class="ek-suggest-meta">'+p.brand+' · '+p.category+'</div></div></div>';}).join('')+'<div class="ek-suggest-item" onclick="saveRecentSearch(\''+escAttr(q)+'\');location.href=\'products.html?q='+encodeURIComponent(q)+'\'">Search for <b>'+q+'</b></div>'; box.style.display='block'; input.setAttribute('aria-expanded','true'); }); }); input.addEventListener('keydown',function(e){ if(e.key==='Enter') saveRecentSearch(input.value.trim()); }); document.addEventListener('click',function(e){ if(e.target!==input && !box.contains(e.target)) box.style.display='none';}); }
function saveRecentSearch(q){ if(!q)return; var arr=ekIds('ek_recent_searches').map(String).filter(function(x){return x.toLowerCase()!==q.toLowerCase();}); arr.unshift(q); localStorage.setItem('ek_recent_searches',JSON.stringify(arr.slice(0,12))); }
